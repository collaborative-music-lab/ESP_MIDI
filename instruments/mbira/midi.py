...
def send_note(note,vel,channel = 0):
    if midi_debug: print('NOTE: ', note,vel)
    if vel > 0:
        if vel > 127: vel = 127
        if note < 0: note = 0
        elif note > 127: note = 127
        midi.send(NoteOn(note,vel),channel) # Send Middle C
    else:
        midi.send(NoteOff(note, 0),channel)
    
cc_values = [0] * 128
def send_cc(num, val,channel = 0):
    val = int(val)
    if val > 127: val = 127
    elif val < 0: val = 0
    if num < 0: num = 0
...
#         if num == 12: print(val)
        cc_values[num] = val
        midi.send(ControlChange(num, int(val)),channel)
        
def force_send_cc(num, val,channel = 0):
    val = int(val)
    if val > 127: val = 127
    elif val < 0: val = 0
    if num < 0: num = 0
