...
class ESPNowPeerManager:
    ...
    def update(self):
        ...
        Stores packets in self.packets
        
        Returns:
            self.available
        """
        if self.espnow is None:  # Check if espnow exists
            print("esp", self.espnow)
            return False
...
#                 print(f"Read attempt {read}: {packet}")
                if packet is None:
                    break
        ...
        if len(packets) == 0:
            return False
        if len(packets) > 1:
            print(f"{len(packets)} packets received")
...
#         print("packet", packets)
        ...
        # parse packets for messages
        msgs = []  # Outer list for all messages
    
        for packet in packets:
            messages = None
            try:
                m = packet.msg.decode("utf-8")
                messages = m.split('!')  # Split commands: ['note|12|127|0', 'note|12|0|100']
            except Exception as e:
            ...
            # Iterate over split messages, append to msgs (not messages!)
            for msg in messages:
                msgs.append({
                    'mac': sender_mac,
                    'data': msg
...
#         print("msg", msgs)  # Now this will print
        ...
        for message in msgs:
            ...
            if packet_obj['type'] == 'ID':
                # Broadcast message (peer discovery)
                device_type = packet_obj["data"]
                name = packet_obj["name"]
                ...
                try:
                    self.add_peer(message["mac"], name, device_type)
        ...
        self.available = len(self.packets)
        return self.available
