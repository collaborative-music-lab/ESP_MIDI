...
enum InstrumentMode { MONOPHONIC,
                      POLYPHONIC };
...
void handleControlChange(uint8_t channel, uint8_t number, uint8_t value) {
  ...
  if (number == 127) {
    ...
    monitorInput = number - 80;
    return;
  }  //monitor raw data
  else if (number >= 60 && number < 80) {
    cap[number - 60].upperThreshold = value * 20;
    return;
  }                                                                                        //update high threshold
  else if (number >= 40 && number < 60) { cap[number - 40].lowerThreshold = value * 20; }  //update low threshold
  else if (number >= 20 && number < 40) {
    cap[number - 20].upperThreshold = value * 20;
  }  //update velocity depth
  ...
  if (millis() - timer > interval) {
    ...
    for (int i = 0; i < 11; i++) {
      ...
      if (SERIAL_DEBUG) {
        ...
        currentSensor = (currentSensor + 1) % 11;
      }
      if (ENABLE_USB_MIDI) {
