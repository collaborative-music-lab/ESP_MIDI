...
// Define a structure for your MIDI data (customize as needed)
...
// Create an instance of your MIDI data structure
struct_midiData midiData;
...
void processIncomingMidi() {
  ...
  if (usbMIDI.readPacket(&midi_packet_in)) {
    ...
    midiData.status = midi_packet_in.byte1;
    midiData.data1 = midi_packet_in.byte2;
    midiData.data2 = midi_packet_in.byte3;

    sendMIDImessageToPeers((uint8_t*)&midiData, sizeof(midiData));
    //esp_err_t result = esp_now_send(broadcastAddress, (uint8_t*)&midiData, sizeof(midiData));
  
  // If sending to a specific peer, replace broadcastAddress with peerAddress
  if(result == ESP_OK){
